    ...
    function finish(save = true) {
        ...
        if (save) {
            ...
            if (val === "") {
                ...
                // Si es un grupo curvo, forzamos la actualización de curvatura [7]
                if (textItem.data?.isCurvedGroup || textItem.data?.curvature) {
                    const curvature = textItem.data.curvature || 0;
                    import("./canvas-pro/textToolbar.js").then(module => {
                        module.applyTextCurve(textItem.parent && textItem.parent.data?.clipGroup ? textItem.parent : textItem, curvature);
                    });
                } else {
