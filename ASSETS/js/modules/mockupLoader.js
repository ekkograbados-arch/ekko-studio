...
function collectPaths(item, paths = []) { 
    ...
    if (item.children) { 
        ...
        children.forEach(function(child) { collectPaths(child, paths); }); 
    } 
    return paths; 
}
 ...
 * 
 * CORRECCIÓN SEGURA: Se quita "militar" para procesar las medallas por silueta de curvas.
 */
function shouldIgnoreLargestPath(paths, rootItem, svgPath = "") { 
    ...
    if (wRatio > 0.95 && hRatio > 0.95) { 
        if (isPathRect(firstPath)) { 
            // Comprobación segura usando operador lógico estricto
...
function isPathRect(path) { 
    ...
    if (path.curves) { 
        hasHandles = path.curves.some(function(c) { return !c.isStraight(); }); 
    } 
...
function buildCompoundMask(item, ignoredPath, svgPath) { 
    ...
    paths.sort(function(a, b) { return Math.abs(b.area) - Math.abs(a.area); }); 
...
function makeMockupTransparent(item, ignoredPath) { 
    ...
    if (item.children) { 
        const children = item.children.slice(); 
        children.forEach(function(child) { makeMockupTransparent(child, ignoredPath); }); 
    } 
}
...
export function loadMockup(svgPath) { 
    ...
    paper.project.importSVG(svgPath, function (item) { 
        ...
        // PASO DE VARIABLE EXPLÍCITO SINTÁCTICAMENTE SEGURO
...
export function restoreMockupReferences() { 
    const mockupItem = paper.project.activeLayer.children.find(function(c) { return c.data && c.data.mockup; }); 
    if (mockupItem) { 
