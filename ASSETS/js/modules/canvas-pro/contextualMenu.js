  ...
  selected.forEach((item, sIdx) => {
    ...
    // C. SI ES COMPOUNDPATH
      ...
      if (item.data?.isOuterWithHoles || activeTarget.data?.isOuterWithHoles) {
        ...
        console.log("%c[EKKO UNGROUP PROCESS] El elemento es un CompoundPath estándar de SVG. Iniciando descomposición jerárquica...", "color: #0284c7; font-weight: bold;");
        const separated = separateContoursIntoIndependentShapes(item);
        if (separated && separated.length > 0) {
          console.log(` - Se generaron ${separated.length} formas sólidas y/o calados celestes reactivos de primer nivel.`);
