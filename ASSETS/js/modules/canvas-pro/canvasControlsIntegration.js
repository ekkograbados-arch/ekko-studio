  ...
  if (!proToolbar) {
    ...
    proToolbar.innerHTML = `
      ...
      <div class="pro-group">
        ...
        <button class="pro-btn" id="proBtnCenterBoth" title="Centrar total en producto"><i class="fas fa-compress-arrows-alt"></i> Total</button>
      </div>
...
const drawDistributionGuides = (selected, axis, spacing) => {
  ...
  window.distributionGuidesGroup.data = { isSelectionBox: true, isSmartGuide: true };
...
// CORRECCIÓN ARQUITECTÓNICA: 'target' es siempre 'displayItem' para que la máscara del producto
// NUNCA sea desplazada ni desfasada del mockup físico al alinear.
const alignSelection = (type) => {
  ...
  if (selected.length === 1) {
    ...
    const bounds = displayItem.bounds;
    // CORRECCIÓN: El target debe ser el diseño real, NUNCA el grupo contenedor de la máscara
    ...
    selected.forEach(item => {
      ...
      const bounds = displayItem.bounds;
      // CORRECCIÓN: Operar sobre displayItem para mantener la máscara del clipGroup estática
      const target = displayItem;
      const oldPos = target.position.clone();
