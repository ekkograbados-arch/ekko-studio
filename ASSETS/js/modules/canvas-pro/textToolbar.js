 ...
 * Ofrece curvatura de textos, soldadura de curvas (Weld), HSpace (espaciado entre letras), negritas y cursivas.
 */
...
export async function loadDynamicFonts() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            loadedFontsCache = fonts;
            return fonts;
        }
    } catch (err) {
        console.error("Error cargando fuentes dinámicas, usando fallbacks:", err);
    }
...
export function applyTextCurve(item, curvature) {
    ...
    const radius = 25000 / curvature; // Curvatura mapeada matemáticamente a un radio de círculo
    const absRadius = Math.abs(radius);
    
    // Calcular el ángulo total ocupado por las letras
    let totalWidth = 0;
    const tempGlyphs = chars.map(char => {
    ...
    let currentAngle = (curvature > 0) ? -totalAngle / 2 : totalAngle / 2;
    
    tempGlyphs.forEach((glyph, idx) => {
        const charW = glyph.bounds.width;
        const charAngle = charW / absRadius;
        const angleStep = charAngle / 2;
        
        const targetAngle = (curvature > 0) ? currentAngle + angleStep : currentAngle - angleStep;
        const x = Math.sin(targetAngle) * absRadius;
        const y = (curvature > 0) ? -Math.cos(targetAngle) * absRadius + absRadius : Math.cos(targetAngle) * absRadius - absRadius;
        
        glyph.position = new paper.Point(x, y);
        glyph.rotate(targetAngle * (180 / Math.PI));
        glyph.fillColor = fillColor;
        
        glyphGroup.addChild(glyph);
        
        currentAngle = (curvature > 0) ? currentAngle + charAngle : currentAngle - charAngle;
    });
        ...
        if (target instanceof paper.PointText) {
            ...
            target.children.forEach(child => {
                if (child instanceof paper.PointText) {
                    const isBold = child.fontWeight === 'bold' || child.fontWeight === 700;
                    child.fontWeight = isBold ? 'normal' : 'bold';
                }
            });
        }
        if (typeof window.updateSelectionBox === 'function') {
