  ...
  window.EKKO_SCENE_STORE = {
    ...
    setAll(states) {
      ...
      Object.keys(states).forEach(key => {
        if (typeof states[key] === "string" && states[key]) sceneStates[key] = states[key];
      });
    },
    has(key) {
    ...
    describe() {
      const product = toolState.currentProduct || null;
      const surfaces = Array.isArray(product?.superficies) ? product.superficies : [];
      const index = Number.isFinite(toolState.currentSurface) ? toolState.currentSurface : 0;
      return {
    ...
    openSurface(productId, surfaceName) {
      ...
      try {
        if (typeof renderSurfacesOnly === "function") renderSurfacesOnly(product);
        loadSurfaceScene(product, product.superficies[index]);
        return true;
      } catch (_) {
        return false;
      }
