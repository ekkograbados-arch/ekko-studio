...
// IMPORTACIÓN ESTÁTICA EXCLUSIVA PARA EL CONTROLADOR DE DOBLADO DE TEXTO
import { applyTextCurve } from "./modules/canvas-pro/textToolbar.js";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  tool.onMouseDrag = function(event) {
    // 2. Dragging dinámico para doblar el texto con el tirador azul (USANDO IMPORTACIÓN ESTÁTICA SEGURA)
    if (window.draggingCurveHandle && window.curveTarget) {
      ...
      const parentItem = window.curveTarget.parent && window.curveTarget.parent.data?.clipGroup ?
        window.curveTarget.parent : window.curveTarget;

      applyTextCurve(parentItem, newCurvature);
      return;
    }
